...
setupCorpusSaver = do
  ...
  where
    ...
    getEventInfo (TestFalsified test) = Just ("reproducers", "unsimplified-", test.reproducer)
    ...
    exceptionHandler workerId env (e :: IOException) = pushEventIO env workerId . ErrorEvent $ "Problem while writing to file: " ++ show e
